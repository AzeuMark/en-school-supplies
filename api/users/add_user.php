...
if (strlen($password) < 8 || !preg_match('/[A-Za-z]/', $password) || !preg_match('/[0-9]/', $password)) $errors[] = 'Password must be 8+ chars with letters and numbers.';
...
$stmt = $pdo->prepare(
    "INSERT INTO users (full_name, email, phone, password, role, status, theme_preference, created_by)
     VALUES (?, ?, ?, ?, ?, 'active', 'auto', ?)"
);
$stmt->execute([$full_name, $email, $phone, aes_encrypt($password), $role, (int)$_SESSION['user']['id']]);
