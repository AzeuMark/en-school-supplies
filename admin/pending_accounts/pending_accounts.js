// Pending Accounts — admin (also used by staff with similar pattern)
(function () {
  ...
  function render(users) {
    if (!users.length) {
      tbl.innerHTML = '<div class="empty-state"><div class="es-icon">✅</div><div class="es-title">No pending accounts</div><div>Newly registered customers will appear here for review.</div></div>';
      return;
    }
    tbl.innerHTML = `<div class="table-wrap"><table class="table">
      <thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Phone</th><th>Registered</th><th>Actions</th></tr></thead>
      <tbody>
        ${users.map(u => `
          <tr>
            <td>#${u.id}</td>
  ...
  tbl.addEventListener('click', async (e) => {
    ...
    if (del) {
      const ok = await Modal.confirm({ title: 'Delete account?', message: `Permanently remove ${del.dataset.name}.`, confirmText: 'Delete', danger: true });
      if (!ok) return;
      try { await EN.api('/api/users/delete_user.php', { body: { id: +del.dataset.del } }); EN.toast('Deleted.', 'success'); load(); } catch (_) {}
    }
  });
