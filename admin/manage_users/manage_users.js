          ...
          <tr>
            ...
            <td><div class="actions">
              ${isSelf
                ...
                : u.status !== 'pending' ? `<button class="btn btn-sm btn-icon btn-secondary" data-edit='${JSON.stringify(u).replace(/'/g,"&#39;")}' title="Edit">✏️</button>` : ''
              }
              ${!isSelf && u.status === 'pending' ? `<button class="btn btn-sm btn-icon action-approve-btn" data-approve="${u.id}" data-name="${EN.escapeHtml(u.full_name)}" title="Approve" aria-label="Approve user"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></button>` : ''}
              ${!isSelf && u.role !== 'admin' && u.status === 'flagged' ? `<button class="btn btn-sm btn-icon action-unflag-btn" data-unflag="${u.id}" data-name="${EN.escapeHtml(u.full_name)}" data-reason="${safeReason}" title="Unflag user" aria-label="Unflag user"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg></button>` : ''}
              ${!isSelf && u.role !== 'admin' && u.status !== 'flagged' && u.status !== 'pending' ? `<button class="btn btn-sm btn-icon btn-warning" data-flag="${u.id}" data-name="${EN.escapeHtml(u.full_name)}" title="Flag">🚩</button>` : ''}
              ${!isSelf && u.role !== 'admin' ? `<button class="btn btn-sm btn-icon btn-danger" data-del="${u.id}" data-name="${EN.escapeHtml(u.full_name)}" title="Delete">🗑️</button>` : ''}
            </div></td>
          </tr>`
        }).join('')}
      </tbody></table></div>`;
