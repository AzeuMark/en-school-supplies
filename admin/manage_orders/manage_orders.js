// Manage Orders — admin (staff version reuses similar code).
(function () {
      ...
      <tbody>
        ${orders.map(o => `
          <tr data-row="${o.id}">
            ...
            <td>
              <div class="actions">
                ${o.status === 'pending' ? `<button class="btn btn-sm" data-action="ready" data-id="${o.id}" title="Mark Ready">✅ Ready</button>` : ''}
                ${(o.status === 'pending' || o.status === 'ready') ? `<button class="btn btn-sm" data-action="claim" data-id="${o.id}" title="Claim with PIN">🎟️ Claim</button>` : ''}
                ${o.status !== 'cancelled' && o.status !== 'claimed' ? `<button class="btn btn-sm btn-secondary" data-action="cancel" data-id="${o.id}" title="Cancel">✖</button>` : ''}
                <a class="btn btn-sm btn-ghost" target="_blank" href="${EN.BASE}/receipt.php?order=${encodeURIComponent(o.order_code)}&pin=${encodeURIComponent(o.claim_pin||'')}" title="View receipt">🧾</a>
                ${role === 'admin' ? `<button class="btn btn-sm btn-danger" data-action="delete" data-id="${o.id}" title="Delete">🗑️</button>` : ''}
              </div>
            </td>
          </tr>`).join('')}
      </tbody>
  ...
  tbl.addEventListener('click', async (e) => {
    ...
    if (tg) {
      const el = document.getElementById('items-' + tg.dataset.toggleItems);
    ...
    if (action === 'ready') {
      ...
      try {
        ...
        EN.toast('Order marked as Ready.', 'success'); load();
      } catch (_) {}
    }
    if (action === 'claim') {
      const bd = Modal.show({
        title: 'Claim Order',
        html: `<p>Ask the customer for their <strong>4-digit Claim PIN</strong> shown on the receipt.</p>
               <div class="field"><label>Claim PIN</label>
                 <input class="input" id="claim-pin" type="text" inputmode="numeric" maxlength="4" pattern="\\d{4}" autofocus></div>`,
      ...
      bd.querySelector('[data-confirm-claim]').addEventListener('click', async () => {
        ...
        try {
          ...
          Modal.close(bd);
          EN.toast('Order claimed.', 'success'); load();
        } catch (_) {}
      });
    }
    if (action === 'cancel') {
      const ok = await Modal.confirm({ title: 'Cancel Order?', message: 'Stock will be restored. This cannot be undone.', confirmText: 'Cancel Order', danger: true });
      if (!ok) return;
      try {
        await EN.api('/api/orders/cancel.php', { body: { order_id: id } });
        EN.toast('Order cancelled.', 'success'); load();
      } catch (_) {}
    }
    if (action === 'delete') {
      const ok = await Modal.confirm({ title: 'Delete Order?', message: 'This permanently removes the order. Stock is restored if not yet claimed.', confirmText: 'Delete', danger: true });
      if (!ok) return;
      try {
        await EN.api('/api/orders/delete.php', { body: { order_id: id } });
        EN.toast('Order deleted.', 'success'); load();
      } catch (_) {}
    }
  });
  ...
  search.addEventListener('input', () => {
    clearTimeout(window._dbo);
    window._dbo = setTimeout(() => { state.q = search.value.trim(); state.page = 1; load(); }, 300);
  });
